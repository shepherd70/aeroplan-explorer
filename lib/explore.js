...
(function (root, factory) {
  ...
  // Pull status of a route: when it was last pulled, how much it covers, and the date window
  // it was pulled for (so a date outside it reads as "not pulled", not "no itineraries").
  // null = never pulled.
  function routeDetail(trips, origin, dest) {
    const r = routeEntry(trips, origin, dest);
    if (!r) return null;
    const dates = Object.values(r.dates || {});
    const w = r.dateWindow;
    return {
      pulledAt: r.pulledAt || null,
      dateCount: dates.length,
      tripCount: dates.reduce((n, d) => n + d.length, 0),
      dateWindow: w && w.start && w.end ? { start: w.start, end: w.end } : null,
    };
  }

  // Why a leg has nothing to show, decided before looking at the day itself:
  //   "no-file"         no trips cache is loaded at all
  //   "not-pulled"      the route was never pulled (routeDetail is null)
  //   "outside-window"  the route was pulled for a window that doesn't include `date`
  //   "ok"              the date is covered — call tripsFor(); an empty list then really means none
  // Files without a recorded window can't tell "outside" from "none", so they report "ok".
  function detailStatus(trips, origin, dest, date) {
    if (!trips) return { status: "no-file", detail: null };
    const detail = routeDetail(trips, origin, dest);
    if (!detail) return { status: "not-pulled", detail: null };
    // A `--date` pull can add a day the full-route window never covered: what is in the file wins.
    if (date && Array.isArray(routeEntry(trips, origin, dest).dates?.[date])) return { status: "ok", detail };
    const w = detail.dateWindow;
    if (w && date && (date < w.start || date > w.end)) return { status: "outside-window", detail };
  ...
  return { CABINS, activeCabins, tripMult, directView, qualifyingCabins, filtered, discoverRows, sweetRows, affordRows,
           ...
           tripsFor, routeDetail, detailStatus, layoverMinutes, cheapestRouting };
});

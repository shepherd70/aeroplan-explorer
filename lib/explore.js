...
(function (root, factory) {
  ...
  function sweetRows(records, settings, opts) {
    const { autoFares = {}, manualFares = {}, today = "" } = opts || {};
    ...
    for (const { rec, q } of filtered(records, settings)) {
      for (const c of q) {
        ...
        if (!prev) {
          ...
          if (c.miles < prev.miles) { prev.miles = c.miles; prev.direct = c.direct; }
        }
      }
    }

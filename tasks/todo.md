...
## Phase 1: Foundation
- [ ] **Task 1** `normalizeTrip()` + schema in `detail.mjs` (isMain-guarded), fixtures + tests in `test/detail.test.mjs` — S
- [ ] **Task 2** `node detail.mjs ORIG-DEST …` CLI: per-route search with `include_trips=true&sources=aeroplan`, skip-pagination, merge-by-route into `trips.cache.json`, quota floor, retries; export `loadApiKey/readRemainingQuota/sleep` from `ingest.mjs` — M
